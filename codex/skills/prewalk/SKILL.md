...
2. When the approach is clear, create a todo list with the plan/todo tool
   ...
   #1 must be the foundational task everything else builds on. *(The edit gate
   will block your `apply_patch` calls until this list exists and every item has
   a checkpoint word — that is intentional.)*
