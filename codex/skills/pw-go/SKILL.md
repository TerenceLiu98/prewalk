...
1. **Switch the session to the executor model now** by running the model-switch
   command the script names (e.g. `/model gpt-5.6-luna`). This is what makes
   prewalk cheap: the executor inherits the whole trajectory — the reads, the
   todo list, and the verified first edit — and finishes the rest.
